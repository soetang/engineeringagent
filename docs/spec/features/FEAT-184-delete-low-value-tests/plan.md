...
- Preserve the coverage contract as an invariant owned by `pyproject.toml:26` and locked by `tests/meta/test_validator.py:1781`. If deleting brittle tests drops coverage for still-important modules, replace that loss with narrower behavior-oriented regressions instead of another helper-only coverage bundle.
...
```python
def test_iteration_run_records_verification_failure(tmp_path: Path) -> None:
...
- Use the full suite after each wave to detect coverage regressions early. If a surviving production surface still lacks meaningful coverage, add or retain the narrowest behavior-facing regression near the anchor suite that already owns that workflow.
...
def test_pytest_default_coverage_contract_is_declared(repo_root: Path) -> None:
    addopts = tomli.loads((repo_root / "pyproject.toml").read_text())["tool"]["pytest"]["ini_options"]["addopts"]
    assert "--cov-fail-under=95" in addopts
```

- Documentation changes: update `docs/spec/features/FEAT-184-delete-low-value-tests/research.md` and `docs/spec/features/FEAT-184-delete-low-value-tests/plan.md` if the actual deleted-file set differs from the current first-wave inventory.
...
```python
def test_cli_validate_surfaces_schema_failures(tmp_path: Path) -> None:
    result = invoke_cli("validate", str(tmp_path))
    assert result.exit_code == 1
    assert "schema" in result.output
```

- Documentation changes: record any intentional coverage replacements in `docs/spec/features/FEAT-184-delete-low-value-tests/research.md` so a later implementation review can distinguish retained anchors from deleted padding.
...
```python
def test_loop_iteration_persists_reviewer_feedback(tmp_path: Path) -> None:
    result = run_iteration(...)
    assert result.reviewers
    assert result.handoff_path.exists()
```

- Documentation changes: update `docs/spec/features/FEAT-184-delete-low-value-tests/research.md` if mixed-file trims reveal dead test-support modules that were not part of the original inventory.
...
- Documentation changes: finalize `docs/spec/features/FEAT-184-delete-low-value-tests/research.md` and `docs/spec/features/FEAT-184-delete-low-value-tests/plan.md` with the implemented deletion waves, retained anchors, and any coverage-driven exceptions.

## Verification Strategy

- Start with targeted anchor suites after each deletion wave so failures quickly reveal which behavior-facing contract lost meaningful coverage.
...
```python
def test_git_commit_message_validation_rejects_invalid_subject() -> None:
...
- Keep `docs/spec/features/FEAT-184-delete-low-value-tests/research.md` as the source-of-truth inventory for deletion candidates, retained anchors, and any mixed-file exceptions discovered during implementation.
- Keep `docs/spec/features/FEAT-184-delete-low-value-tests/plan.md` aligned with the actual deletion wave order, final retained anchor suites, and verification commands used for the implementation session.
