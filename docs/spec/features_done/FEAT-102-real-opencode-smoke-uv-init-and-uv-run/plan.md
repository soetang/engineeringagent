...
Update harness/fitness-functions/real_opencode_hello_world_feature_template.yaml so all verification commands use `uv run python` and the template text reflects that uv init creates a minimal pyproject.toml which must not be modified.

## ST-002 Run uv init in the smoke temp repo and switch post-run verification to uv

Update harness/fitness-functions/check_real_opencode_hello_world_smoke.py to run `uv init . --package --vcs none --no-readme --no-pin-python` in the temp repo root and to run all post-run verification commands via `uv run python`.
