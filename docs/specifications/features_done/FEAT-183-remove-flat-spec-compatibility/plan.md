...
- Centralize the bundled-only path rule in `iter_feature_files()`, `feature_storage_root()`, `resolve_feature_package_paths()`, and `feature_progress_kind()` before updating `src/engineeringagent/prompts/renderer.py`, `harness/reviewers/prompts/intent_integrity_reviewer.md`, `harness/reviewers/prompts/test_reviewer.md`, `harness/fitness_functions/check_source_first_loop_commands.py`, and `harness/fitness_functions/check_real_opencode_hello_world_smoke.py`; this removes repeated flat-path branching.
...
- Goal: delete every `docs/spec/features_done/*.yaml` file, remove flat-path handling from `harness/checks.yaml`, `harness/fitness_functions/check_source_first_loop_commands.py`, and `harness/fitness_functions/check_real_opencode_hello_world_smoke.py`, and add feature-local scripts that fail if those paths or phrases reappear.
- Progress update (2026-03-10): bundled-only repo enforcement now rejects top-level flat `*.yml` entrypoints in addition to `*.yaml`; `run_repo_validation()`, `check_no_flat_feature_specs.py`, and focused bundled-layout tests now share that stricter absence rule.
...
- Progress update (2026-03-10): `check_source_first_loop_commands.py` no longer scans the archived FEAT-181 `plan-format-example.md`; source-first command enforcement is now limited to active feature bundles, packaged approach docs, contributor docs, prompt templates, checks config, and the live smoke fixture.
- Progress update (2026-03-10): the archived FEAT-181 support examples (`supporting/spec-format-example.yaml` and `supporting/plan-format-example.md`) were deleted, and the remaining spec-writing regression coverage now anchors on the live smoke feature template and plan fixture instead of archived example artifacts.
- Progress update (2026-03-10): loop/reviewer runtime regression suites now model archived features only as bundled done-package entrypoints (`docs/spec/features_done/<feature>/spec.yaml`); the stale flat `features_done/*.yaml` fixtures were removed from `tests/loop/test_loop_reviewers.py`, `tests/loop/test_loop_phases_coverage.py`, `tests/harness/test_checks_runtime.py`, and reviewer runtime/sandbox tests.
...
- Interfaces:
  ...
  - Update `_parse_feature_statuses()` in `harness/fitness_functions/check_real_opencode_hello_world_smoke.py` to stop falling back to flat `subtasks`.
