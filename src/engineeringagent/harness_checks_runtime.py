...
def run_planned_command_checks(
    *,
    ...
    planned = plan_command_checks(doc, phase=phase, changed_paths=changed_paths)
    combined_output_parts: list[str] = []
    timings: list[CommandTiming] = []

    for check_id, command in iter_planned_command_check_commands(doc, planned):
        started_epoch_sec = int(time.time())
        proc = run_shell_command(project_root, command)
