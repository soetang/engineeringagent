...
from engineeringagent.scaffolding.models import ScaffoldFile
...
- Run package commands with `engineeringagent ...`.
- Initialize a repository scaffold with `engineeringagent init`.
- Export machine-readable contracts with `engineeringagent schema plan` and `engineeringagent schema quality`.
- Validate plans before implementation with `engineeringagent validate-plan <plan.md>`.
- Run checks with `engineeringagent check validate` and `engineeringagent check run`.
- Start an implementation loop with `engineeringagent implement <plan.md>`.
