...
def write_set_subtask_done_script(script_path: Path, subtask_id: str) -> Path:
    script_path.write_text(
        "\n".join(
            [
                "from pathlib import Path",
...
def write_add_done_subtask_script(
    script_path: Path,
    subtask_id: str,
...
def write_set_done_and_duplicate_subtask_script(
    script_path: Path,
    subtask_id: str,
