...
def test_tests_do_not_import_engineeringagent_commit_messages() -> None:
    repo_root = Path(__file__).resolve().parents[1]
...
def test_commit_subject_pattern_generated_from_allowed_types() -> None:
    repo_root = Path(__file__).resolve().parents[1]
...
def test_validate_commit_subject_accepts_allowed_format(tmp_path: Path) -> None:
    ...
    for subject in [
        ...
        proc = _run_validator("--commit-msg-file", str(commit_file))
        assert proc.returncode == 0
        assert "commit message validation: ok" in proc.stdout


def test_validate_commit_subject_rejects_invalid_prefix(tmp_path: Path) -> None:
    commit_file = tmp_path / "COMMIT_EDITMSG"
...
def test_commit_msg_file_mode_skips_comments(tmp_path: Path) -> None:
    commit_file = tmp_path / "COMMIT_EDITMSG"
...
def test_commit_range_mode_reports_deterministic_errors(tmp_path: Path) -> None:
    repo = tmp_path / "repo"
    _init_repo(repo)
...
def test_commit_range_mode_ok_for_valid_subjects(tmp_path: Path) -> None:
    repo = tmp_path / "repo"
    _init_repo(repo)
...
def test_commit_range_mode_raises_for_invalid_range(tmp_path: Path) -> None:
    repo = tmp_path / "repo"
    _init_repo(repo)
