    ...
    _write_module(
        ...
        "\n".join(
            [
                ...
                "    return project_root / 'progress' / 'runs.jsonl'",
                "",
                "def run_feature_log_path(project_root: Path, feature_id: str) -> Path:",
                "    return project_root / 'progress' / f'run-feature-{feature_id}.txt'",
            ]
        )
        + "\n",
    )
...
def test_logging_path_locality_checker_emits_expected_rule_id(tmp_path: Path) -> None:
...
def test_logging_path_locality_rule_fails_on_inline_progress_path_literal(
    ...
    assert any(
        "src/engineeringagent/loop_runtime/telemetry.py:1 contains progress artifact path literal 'runs.jsonl'"
        in violation
    ...
    _write_module(
        ...
        "\n".join(
            [
                ...
                "from pathlib import Path",
                "",
                "from engineeringagent.progress_paths import runs_jsonl_path",
                "",
                "def emit() -> None:",
...
def test_logging_path_locality_rule_passes_for_repo() -> None:
