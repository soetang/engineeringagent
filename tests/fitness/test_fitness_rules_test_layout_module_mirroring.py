...
def test_test_layout_module_mirroring_rule_flags_flat_git_worktree_manager_test(
    tmp_path: Path,
    repo_root: Path,
) -> None:
    """Reject the legacy flat vcs adapter test after introducing a mirrored path."""
    _write_file(tmp_path, "tests/adapters/test_git_worktree_manager.py", "")
    _write_file(tmp_path, "tests/__init__.py", "")
    _write_file(tmp_path, "tests/conftest.py", "")
    _write_file(tmp_path, "src/engineeringagent/adapters/__init__.py", "")
    _write_file(tmp_path, "src/engineeringagent/adapters/vcs/git_worktree_manager.py", "")
    ...
    assert result["status"] == "fail"
    assert _violations(result) == [
        "tests/adapters/test_git_worktree_manager.py: legacy test path is forbidden; move it under the mirrored source module path."
    ]


def test_test_layout_module_mirroring_rule_flags_legacy_harness_checks_runtime_test(
    tmp_path: Path,
    repo_root: Path,
) -> None:
    """Reject the removed harness checks-runtime test path."""
    _write_file(tmp_path, "tests/harness/test_checks_runtime.py", "")
    _write_file(tmp_path, "tests/__init__.py", "")
    _write_file(tmp_path, "tests/conftest.py", "")
    _write_file(tmp_path, "src/engineeringagent/adapters/runtime/iteration_phases.py", "")
    ...
    assert result["status"] == "fail"
    assert _violations(result) == [
        "tests/harness/test_checks_runtime.py: legacy test path is forbidden; move it under the mirrored source module path."
    ]
...
def test_test_layout_module_mirroring_rule_flags_legacy_flat_workspace_test_paths(
    ...
    """Reject workspace-subpackage tests after moving services to root modules."""
    ...
    assert result["status"] == "fail"
    assert _violations(result) == [
        "tests/application/workspace/test_init_service.py: not mirrored by src module path src/engineeringagent/application/workspace",
        "tests/application/workspace/test_recovery_service.py: not mirrored by src module path src/engineeringagent/application/workspace",
    ]
...
def test_test_layout_module_mirroring_rule_allows_mirrored_workspace_tests(
    ...
    """Allow mirrored tests for root-level workspace service modules."""
