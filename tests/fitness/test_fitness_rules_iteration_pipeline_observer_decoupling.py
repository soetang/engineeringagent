...
def test_iteration_pipeline_observer_decoupling_rule_fails_on_console_and_telemetry_calls(
    ...
    assert any(
        "src/engineeringagent/application/feature_iteration_runtime/pipeline.py:4 invokes console output sink 'print'"
        in violation
        for violation in violations
    )
    assert any(
        "src/engineeringagent/application/feature_iteration_runtime/pipeline.py:5 invokes console output sink 'print_summary'"
        in violation
        for violation in violations
    )
    assert any(
        "src/engineeringagent/application/feature_iteration_runtime/pipeline.py:6 invokes telemetry sink 'write_iteration_telemetry'"
        in violation
        for violation in violations
    )
