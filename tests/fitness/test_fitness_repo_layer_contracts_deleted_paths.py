...
def test_repo_layer_contracts_rule_blocks_deleted_application_contracts_directory(
    ...
    (contracts_root / "checks.py").write_text("class RunChecksRequest:\n    pass\n", encoding="utf-8")
...
def test_repo_layer_contracts_rule_blocks_deleted_top_level_init_scaffold_module(
    tmp_path: Path,
    repo_root: Path,
) -> None:
    """Fail when scaffold helpers drift back to the package root."""
    legacy_module = tmp_path / "src" / "engineeringagent" / "init_scaffold.py"
    legacy_module.parent.mkdir(parents=True, exist_ok=True)
    legacy_module.write_text("", encoding="utf-8")
    ...
    assert payload["rule_id"] == "architecture.repo-layer-contracts"
    assert payload["violations"] == [
        "src/engineeringagent/init_scaffold.py: deleted legacy module path must remain absent"
    ]


def test_repo_layer_contracts_rule_blocks_deleted_quality_repository_validator_module(
    tmp_path: Path,
    repo_root: Path,
) -> None:
    """Fail when the removed flat quality repository-validator module reappears."""
    legacy_module = (
        tmp_path
        / "src"
        / "engineeringagent"
    ...
    assert payload["rule_id"] == "architecture.repo-layer-contracts"
    assert payload["violations"] == [
        "src/engineeringagent/adapters/quality/repository_validator.py: deleted legacy module path must remain absent"
    ]
...
def test_repo_layer_contracts_rule_blocks_deleted_document_feature_selection_module(
    tmp_path: Path,
    repo_root: Path,
) -> None:
    """Fail when the removed feature-selection document adapter returns."""
    legacy_module = (
        tmp_path
        / "src"
        / "engineeringagent"
        / "adapters"
        / "documents"
        / "filesystem_feature_selection.py"
    )
    legacy_module.parent.mkdir(parents=True, exist_ok=True)
    legacy_module.write_text("", encoding="utf-8")
    ...
    assert payload["rule_id"] == "architecture.repo-layer-contracts"
    assert payload["violations"] == [
        "src/engineeringagent/adapters/documents/filesystem_feature_selection.py: deleted legacy module path must remain absent"
    ]
...
def test_repo_layer_contracts_rule_blocks_deleted_feature_iteration_contracts_module(
    tmp_path: Path,
    repo_root: Path,
) -> None:
    """Fail when the removed nested feature-iteration contracts module reappears."""
    legacy_module = (
        tmp_path
        / "src"
        / "engineeringagent"
    ...
    assert payload["rule_id"] == "architecture.repo-layer-contracts"
    assert payload["violations"] == [
        "src/engineeringagent/application/feature_iteration/contracts.py: deleted legacy module path must remain absent"
    ]
...
def test_repo_layer_contracts_rule_allows_root_init_workspace_service_module(
    tmp_path: Path,
    repo_root: Path,
) -> None:
    """Allow the canonical root init workspace service module."""
    module_path = (
        tmp_path
        / "src"
        / "engineeringagent"
...
def test_repo_layer_contracts_rule_allows_root_workspace_recovery_service_module(
    tmp_path: Path,
    repo_root: Path,
) -> None:
    """Allow the canonical root workspace recovery service module."""
    module_path = (
        tmp_path
        / "src"
        / "engineeringagent"
...
def test_repo_layer_contracts_rule_blocks_deleted_workspace_application_package(
    tmp_path: Path,
    repo_root: Path,
) -> None:
    """Fail when the removed application workspace package reappears."""
