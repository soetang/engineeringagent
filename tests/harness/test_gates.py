...
def _repo_root() -> Path:
    ...
    for candidate in (here, *here.parents):
        if (candidate / "pyproject.toml").exists():
            return candidate
    raise RuntimeError("could not resolve repo root from __file__")
...
def test_empty_profile_returns_friendly_success_message(
    ...
    code = cmd_gates_run(
        SimpleNamespace(project_root=str(tmp_path), profile="precommit")
    )
...
def test_cmd_gates_run_output_behavior_unchanged(tmp_path: Path, capfd: Any) -> None:
    ...
    code = cmd_gates_run(
        SimpleNamespace(project_root=str(tmp_path), profile="loop_fast")
    )
...
def test_main_uses_typer_gates_tree_without_legacy_forward(monkeypatch: Any) -> None:
    ...
    def _fake_cmd_gates_run(args: SimpleNamespace) -> int:
