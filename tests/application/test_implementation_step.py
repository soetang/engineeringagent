...
def test_run_implement_step_emits_start_and_output_through_runtime_callbacks() -> None:
    ...
    runner = _FakeAgentRunner(
        ImplementProgressEnvelope(
            ...
            verification=["uv run pytest tests/application/test_implementation_step.py"],
    ...
    assert observed["outputs"] == [
        '{"blockers": [], "completed_work": ["moved implement-step status output to callbacks"], "remaining_work": ["run the full checks phase"], "summary": "updated implementation step", "verification": ["uv run pytest tests/application/test_implementation_step.py"]}'
    ]
